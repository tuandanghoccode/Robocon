...
void testTriangles()
{
    int cx = _width  / 2 - 1;
    int cy = _height / 2 - 1;
    int size = min(cx, cy)*0.9; // Kích thước tam giác

    fillScreen(WHITE);

    int x1 = cx,     y1 = cy - size; // Đỉnh trên
    int x2 = cx - size, y2 = cy + size; // Góc trái dưới
    int x3 = cx + size, y3 = cy + size; // Góc phải dưới

    uint16_t fillColor = color565(255, 255, 0);  // Màu tam giác (vàng)
    uint16_t borderColor = color565(255, 0, 0); // Màu viền (đỏ)

    // Vẽ tam giác đầy màu
    fillTriangle(x1, y1, x2, y2, x3, y3, fillColor);

    // Vẽ viền dày hơn bằng cách vẽ nhiều lần với các giá trị hơi lệch
    for (int d = -2; d <= 2; d++) { // Điều chỉnh độ dày ở đây
        drawTriangle(x1, y1 + d, x2 + d, y2, x3 - d, y3 , borderColor);
    }
}

void drawHeart()
{
    int cx = _width / 2;  // Tọa độ trung tâm (x)
    int cy = _height / 2; // Tọa độ trung tâm (y)
    int size = min(cx, cy) * 0.065; // Điều chỉnh kích thước trái tim

    fillScreen(color565(255, 182, 193)); // Nền màu hồng

    uint16_t fillColor = color565(255, 0, 0);  // Màu viền (đỏ)
    uint16_t borderColor = color565(255, 0, 0); // Màu viền đậm hơn (đỏ)

    for (float t = 0; t < 2 * 3.14; t += 0.01) {
        int x = cx + size * (16 * pow(sin(t), 3));
        int y = cy - size * (13 * cos(t) - 5 * cos(2 * t) - 2 * cos(3 * t) - cos(4 * t));

        drawPixel(x, y, fillColor); // Vẽ từng điểm của trái tim
    }

    // Tạo viền dày hơn bằng cách vẽ nhiều lần với các giá trị hơi lệch
    for (int d = -2; d <= 4; d++) {
        for (float t = 0; t < 2 * 3.14; t += 0.01) {
            int x = cx + size * (16 * pow(sin(t), 3)) + d*0.2;
            int y = cy - size * (13 * cos(t) - 5 * cos(2 * t) - 2 * cos(3 * t) - cos(4 * t)) + d*0.2;
